...
function showItem(i) {
  ...
  el.img.onload = () => {
    ...
    decoded.then(() => {
      if (S.items[S.idx] !== it || !S.running) return;   // a late load for an image already answered
      el.loading.hidden = true; el.stage.classList.remove('waiting');
      // start the twelve seconds on the first frame that actually shows the image
      requestAnimationFrame(() => requestAnimationFrame(() => {
        if (S.items[S.idx] !== it) return;
        S.shownAt = performance.now(); S.accepting = true;
